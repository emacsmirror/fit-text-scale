...
There are cases when the automatism fails.  Use ~C-g~ to break the
automatism.  Use the classical text-scale functions to increase and
decrease the scale.  Try keys ~C-x C-+~ and ~C-x C--~.
...
- C-x C-& binds to ~fts-max-font-size-fit-line~.
- C-u C-x C-& binds to ~fts-max-font-size-fit-lines~.
- C-x C-* binds to ~fts-max-font-size-fit-buffer~.
